...
You may consider using the command `cat gateway.yaml | sed 's/${CLUSTER_NAME}/cluster1.test.com/g' | oc apply -f -` in order to replace the cluster domain.
...
You may consider using the command `cat httproute.yaml | sed 's/${CLUSTER_NAME}/cluster1.test.com/g' | oc apply -f -` in order to replace the cluster domain.
...
You may consider using the command `cat grpcroute.yaml | sed 's/${CLUSTER_NAME}/cluster1.test.com/g' | oc apply -f -` in order to replace the cluster domain.
